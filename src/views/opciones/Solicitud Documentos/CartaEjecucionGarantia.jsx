...
const CartaEjecucionGarantia = () => {
  ...
  const navigate = useNavigate();
    ...
    const {
      ...
      const [flagJudicial, setFlagJudicial] = useState(0); // Estado para el flag
      const [numCarta, setNumCarta] = useState(3);
...
// USSEeFECT PARA LA FUNCION DE VALIDAR SI NO ES DEL TIPO JUDICIAL

useEffect(() => {
  const verificaJudicial = () => {
    // Verifica si "JUDICIAL" o "EXP-JUD" está en la cadena de agencia
    if (productoscliente.AGENCIA.includes("JUDICIAL") || productoscliente.AGENCIA.includes("EXP-JUD")) {
      setFlagJudicial(1); // Establece flagJudicial a 1
    } else {
      setFlagJudicial(0); // Establece flagJudicial a 0
    }
  };

  verificaJudicial(); // Llama a la función
  // eslint-disable-next-line react-hooks/exhaustive-deps
}, [productoscliente]); // Dependencia: se ejecuta si cambia `agencia`
      ...
      return (
        flagJudicial===0 ? (
          ...
          <div className="flex justify-between w-full items-center ">
            <h1 className="text-xl pl-3 font-semibold">CARTA EJECUCIÓN GARANTÍA</h1>
            <div className="flex flex-col gap-2 items-center">
              <label> NUM CARTA</label>
...
export default CartaEjecucionGarantia