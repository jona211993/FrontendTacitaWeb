...
const FormularioEditarReclamo = ({ selectRow ,closeModal }) => {
  const { user } = useData();
//   const navigate = useNavigate();
  const [idReclamo, setIdReclamo] = useState(0);
  ...
  useEffect(() => {
    ...
    setIdReclamo(selectRow?.idReclamo)
    setFechaReclamo(selectRow?.fecReclamo);
    setFechaSuceso(selectRow?.fecSuceso);
    setViaReclamo(selectRow?.via);
    setDniReclamo(selectRow?.DNI_RECLAMO);
    setNombre(selectRow?.nombreReferencial);
    setEsCliente(selectRow?.esCliente);
    if(selectRow?.esCliente==="Si"){
      setCheckDniDeudor(false);
      setHabilitarBtnValidar(false);
      setValidar2(true);
      console.log("validar: ", validar1, validar2)
    }
    setDniDeudor(selectRow?.DNI_DEUDOR);
    if(selectRow?.DNI_DEUDOR==="SIN DNI"){
      setCheckDniDeudor(false);
      setHabilitarBtnValidar(false);
      setValidar2(true);
      console.log("validar: ", validar1, validar2)
    }
    setSeResolvio(selectRow?.estadoReclamo === "Resuelto" ? "Si" : "No");
    setCanal(selectRow?.canal);
    setMotivo(selectRow?.motivo);
    setObservacion(selectRow?.observacion);
  }, []);
  ...
  const handleChangeCheckDniDeudor = (e) => {
    ...
    if (checkDniDeudor===false && selectRow.esCliente==="No") {
      setDniDeudor("");
      setValidar2(false)      
      
    } else if(checkDniDeudor===true && esCliente==="No") {
      setDniDeudor("");      
      setHabilitarCheckDniDeudor(true);
      setHabilitarInputDniDeudor(true);
      setHabilitarBtnValidar(true);
  ...
  const handleRegistrar = async () => {
    if (validateFields()) {
      const dataToSend = {
        ...
        estadoReclamo: selectRow?.estadoReclamo,
        canal: canal,
        motivo: motivo,
        usuario: user.alias,
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/crearReclamo`,
          dataToSend
        );
    ...
    // Cerrar el modal de edición después de un breve retraso
    setTimeout(() => {
      closeModal();
      window.location.reload(); // Recargar la página
        ...
        //   }, 2000);
          ...
          showModalError(
            "Hubo un problema al registrar el reclamo. Intenta nuevamente."
          );
        }
      } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-5 mt-5 pl-5 pr-5 font-semibold">
      ...
      <div className="flex w-full items-center justify-center mt-5 gap-6">
        <Button
          type="primary"
        ...
        <Button className="bg-white text-red-500 border-red-500"  onClick={closeModal}>
          CANCELAR
        </Button>
        <Modal
        ...
        >
          <Result
            ...
            subTitle="Se hicieron los cambios en la información del reclamo"
          />
        </Modal>
      </div>
    </div>
...
export default FormularioEditarReclamo;
